...
if [[ $EUID -eq 0 ]]; then
   ...
   echo -e "${BLUE}[INFO]${NC} Use 'sudo ./run-macos.sh' for full functionality"
fi
...
if [ $? -ne 0 ]; then
    ...
    echo -e "${BLUE}[INFO]${NC} Run ./install-macos.sh first if you haven't already"
    echo ""
fi
